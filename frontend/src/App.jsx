        ...
        <div className="bg-background text-white h-[100dvh] w-full overflow-y-scroll snap-y snap-mandatory scroll-smooth no-scrollbar overscroll-y-contain">
            ...
            {sections.map((section, index) => (
                <section
                    key={index}
                    id={`section-${index}`}
                    className={`w-full snap-start snap-always relative flex items-center justify-center overflow-x-hidden
                        ${index === 0 ? 'min-h-screen md:h-screen' : 'min-h-screen py-16 md:py-0'} 
                    `}
